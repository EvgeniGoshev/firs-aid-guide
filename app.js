...
async function loadRemoteProgress() {
  ...
  progress = {
    ...
    teamName: profile?.team_name || progress.teamName || "Student first aid team",
...
function renderAccount() {
  ...
  const teamName = `${progress.teamName || "Student first aid team"} · ${progress.role || "Competitor"} · ${backendLabel}`;
...
document.getElementById("account-form").addEventListener("submit", async (event) => {
  ...
  if (currentUser) {
    await supabaseClient.from("profiles").upsert({
      id: currentUser.id,
      display_name: name,
      team_name: team || "Student first aid team",
    ...
    progress = {
      ...
      email,
      profileName: name,
      teamName: team || "Student first aid team",
      initials: (initials || name.slice(0, 2) || "FA").toUpperCase(),
      role,
      heardFrom
    };
